...
int main(int argc, char **args)
{
    if (argc != 4)
    {
        std::cerr << "[ERROR]: Invalid parameters. 
                     Parameters are : [server_addr][port][sens_threshold]
                                          .Exiting." << std::endl;
    ...
    {
        std::cerr << "[ERROR]: Function connect() failed with error code: "
    ...
    if (sensBytes == -1)
    {
        std::cerr << "[ERROR]: Failed to send sensitivity threshold with error code: "
                  << std::strerror(errno) << std::endl;
        close(clientSocket);
        return (int)errno;
    }
